...
<?php if ($focus === 'topics'): ?>
<section class="sg-dashboard-section"><h2>Themen</h2>
...
<?php endforeach ?></div><?php else: ?><p class="sg-muted">Noch sind keine Themen vergeben. Kreise können sie beim Erstellen oder Bearbeiten eines Vorhabens ergänzen.</p><?php endif ?>
</section>
<section class="sg-dashboard-section"><h2><?= $selectedTopic ? 'Vorhaben zum Thema „' . Html::encode($selectedTopic) . '“' : 'Alle Ideen und Aufgaben' ?></h2>
...
<article class="sg-resource <?= Html::encode($item->circle ? $item->circle->colorClass() : 'sg-tone-teal') ?>"><?= Html::a('', $item->space->createUrl('/sociocratic-governance/work/view', ['id' => $item->id, 'section' => 'resources']), ['class' => 'sg-card-hit', 'aria-label' => 'Ressourcen von „' . $item->title . '“ öffnen']) ?><span class="sg-eyebrow"><?= Html::encode(\humhub\modules\sociocraticGovernance\models\WorkResource::TYPES[$resource->resource_type]) ?> · <?= Html::encode($item->space->name) ?></span>
<h3><?= Html::a(Html::encode($resource->label), $item->space->createUrl('/sociocratic-governance/work/view', ['id' => $item->id, 'section' => 'resources'])) ?></h3>
<p>Für: <?= Html::a(Html::encode($item->title), $item->space->createUrl('/sociocratic-governance/work/view', ['id' => $item->id, 'section' => 'resources'])) ?></p>
