...
foreach ($nodes as $id => $node) {
    ...
    $graph['nodes'][] = [
        ...
        'url' => (string) $circle->space->createUrl('/sociocratic-governance/circle/index'),
...
<section class="sg-card" data-sg-directory-panel="table"<?= $activeView === 'table' ? '' : ' hidden' ?>><div class="table-responsive"><table class="sg-directory-table"><thead><tr><th>Kreis</th><th>Mandat in Kürze</th><th>Rollen</th><th>Weitere Mitglieder</th></tr></thead><tbody>
<?php foreach ($rows as $row): $circle = $row['circle']; ?>
<tr class="<?= Html::encode($circle->colorClass()) ?>"><td><div class="sg-tree-name" style="padding-left:<?= (int) $row['depth'] * 28 ?>px"><?= Html::a(Html::encode($circle->space->name), $circle->space->createUrl('/sociocratic-governance/circle/index')) ?></div></td><td><?= Html::encode($circle->mandateSummary() ?: 'Noch nicht beschrieben.') ?></td><td class="sg-role-avatars"><?= $roleImages($circle) ?></td><td><?= $otherMembers($circle) ?></td></tr>
...
<section class="sg-card sg-directory-map-panel" data-sg-directory-panel="map"<?= $activeView === 'map' ? '' : ' hidden' ?>><div class="sg-map-intro"><p class="sg-muted">Kreise öffnen ihre Details und Mitglieder direkt in der Karte. Ziehen verschiebt die Ansicht; Mausrad oder die Schaltflächen zoomen. Falls die interaktive Karte nicht verfügbar ist, bleibt die strukturierte Übersicht sichtbar.</p></div><div class="sg-circle-map sg-map-fallback-mode" data-sg-circle-map data-graph='<?= Json::htmlEncode($graph) ?>' aria-label="Arbeitskreis-Karte" tabindex="0"><div class="sg-map-fallback"><?php foreach ($rows as $row): $circle = $row['circle']; ?><article class="sg-map-fallback-node <?= Html::encode($circle->colorClass()) ?>" style="margin-left:<?= min(112, (int) $row['depth'] * 28) ?>px"><span class="sg-eyebrow">Arbeitskreis<?= $row['depth'] ? ' · Ebene ' . ((int) $row['depth'] + 1) : '' ?></span><h3><?= Html::a(Html::encode($circle->space->name), $circle->space->createUrl('/sociocratic-governance/circle/index')) ?></h3><p><?= Html::encode($circle->mandateSummary() ?: 'Mandat noch nicht beschrieben.') ?></p><p class="sg-muted"><?= count(\humhub\modules\sociocraticGovernance\services\CircleDirectory::people($circle)) ?> sichtbare Mitglieder</p></article><?php endforeach ?></div></div></section>
